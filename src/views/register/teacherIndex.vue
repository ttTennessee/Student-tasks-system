<template lang="html">
  <div align="center">
    ...
    <el-form ref="form" :model="form" :rules="registerRules" class="login-form" autocomplete="on" label-position="center">
      ...
      <el-form-item prop="subject" style="width: 380px">
          <span class="svg-container">
          </span>
        <el-input
          ref="form.subject"
  ...
  export default {
    ...
    methods: {
      sendCode() {
        this.$refs.form.validate(valid => {
          if (valid) {
            ...
            this.timer()
            this.loading = true
            this.$store.dispatch('teacher/sendCode', this.form)
              .then(() => {
                alert("发送验证码成功")
      ...
      studentRegister() {
        this.$refs.form.validate(valid => {
          if (valid) {
            this.$store.dispatch('teacher/teacherRegister', this.form)
              .then(() => {
                this.loading = false
              })
              .catch(() => {
                this.$router.push({ path: '/teacher/login', query: this.otherQuery })
                this.loading = false
              })
          } else {
            console.log('error submit!!')
      ...
      onSubmit() {
        const _this = this
        axios.post('/teacher/register',qs.stringify({
      ...
      getCode() {
        axios.post('/teacher/sendCode',qs.stringify({
